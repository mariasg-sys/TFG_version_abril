...
"""
Graficas para entrenamiento 3D.

Uso recomendado:
    python plot_training_history_3d.py "D:/Clase/TFG/versión abril/amos_3d/runs/run_xxx/training_history_3d.csv"

Si no pasas ruta, busca automaticamente el run mas reciente en:
    D:/Clase/TFG/versión abril/amos_3d/runs
"""
...
RUNS_ROOT = Path("D:/Clase/TFG/versión abril/amos_3d/runs")
HISTORY_NAME = "training_history_3d.csv"
...
def dice_plot(df, output_dir):
    ...
    ax.set_title("Dice por clase y Mean Dice - U-Net 3D")
    ax.set_xlabel("Epoch")
    ax.set_ylabel("Dice")
    ax.set_ylim(0, 1)
    ax.grid(True, alpha=0.3)
    ...
    fig.tight_layout()
    out = output_dir / "dice_by_class_3d.png"
...
def main():
    ...
    print(f"Leyendo historial 3D: {history_path}")
    df = read_history(history_path)
    output_dir = history_path.parent / "plots_3d"
    output_dir.mkdir(parents=True, exist_ok=True)

    metric_plot(df, output_dir, "train_loss", "Train Loss - U-Net 3D", "Train Loss", "min")
    metric_plot(df, output_dir, "val_loss", "Validation Loss - U-Net 3D", "Validation Loss", "min")
    metric_plot(df, output_dir, "voxel_accuracy", "Voxel Accuracy - U-Net 3D", "Voxel Accuracy (%)", "max")
    dice_plot(df, output_dir)
    print(f"\nGraficas 3D generadas en: {output_dir}")
