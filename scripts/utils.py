...
def check_accuracy(loader, model, num_classes, device="cuda"):
    ...
    with torch.no_grad():
        for x, y in tqdm(loader, desc="Calculando Dice/accuracy"):
            x = x.to(device)
            y = y.to(device)
            logits = model(x)
            preds = torch.argmax(logits, dim=1)
    ...
    return mean_dice, pixel_acc, dice_by_class
...
def validation_function(model, loader, loss_fn, device):
    ...
    with torch.no_grad():
        for data, targets in tqdm(loader, desc="Validacion"):
...
def save_predictions_as_imgs(loader, model, folder, device, max_batches=4):
    ...
    with torch.no_grad():
        for idx, (x, y) in enumerate(tqdm(loader, desc="Guardando predicciones")):
            if idx >= max_batches:
                break
